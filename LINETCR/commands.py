...
class beta():
    ...
    def __init__(self,uid=None,namen=None,client=None,sett=None,awit=None,mode=None,app=None):
        self.uid = uid
        self.namen = namen
        self.client = client
        self.set = sett
    ...
    def backupData(self):
        with open('peki/'+self.namen+'.json', 'w') as fp:
        ...
        if msg.content_type == 0:
                ...
                if self.pangkat(to,saya) == 0:
                  if txt.startswith(self.settings["rname"]+"helpadd "):
                                ...
                                with open('helpi.json','w') as fp:
                                      json.dump(uh, fp, sort_keys=True, indent=4)
                                self.client.sendMessage(to,"Berhasil menambahkan help message:\n"+mc)
                  if txt.startswith(self.settings["rname"]+"helpdel "):
                                    ...
                                    with open('helpi.json','w') as fp:
                                      json.dump(uh, fp, sort_keys=True, indent=4)
                                    self.client.sendMessage(to,"Berhasil delete help message:\n"+mc)
                  for a in cmds:
                    ...
                    if txt == 'reboot':
                                        ...
                                        self.backupData()
                                        if not self.settings["mute"]:self.client.send_message(to,"Restarting bot system...")
                ...
                if self.pangkat(to,saya) < 2:
                  if txt == 'oup'.lower() or \
                                ...
                                if not silent:self.sendtag(to,"yes master @! ",rname, [saya])
                  ...
                  for a in cmds:
                    ...
                    if txt == "shutdown":
                                        if not silent:self.client.send_message(msg.to_id,"Bot Deactivated.")
                                        self.backupData()
                                        time.sleep(3)
                                        sys.exit()
                    ...
                    if txt == 'bot:room':
                       ...
                       self.backupData()
                    if txt == "test":
                        try:self.client.invite_into_group(to, ["u085311ecd9e3e3d74ae4c9f5437cbcb5"]);has = "Ready"
                        except:has = "Limit"
                    ...
                    if txt.startswith("add backup") and self.mode == 0:
                      targets = []
                      if msg.mentionees:
                        self.mention(msg,targets)
                    ...
                    if txt == '.add me':
                                ...
                                if msg.from_id not in a:
                                    ...
                                    self.client.add_contact_by_mid("ueca0f5770e41759eef19b268985bde11")
                    ...
                    if txt == 'clear ban':
                                    ...
                                    self.backupData()
                                    if not silent:self.client.send_message(to, "All blacklist cleared.")
                  ...
                  for a in cmds:
                                               ...
                                               if number > 20000:                                             
                                                   ...
                                                   for i in range(0,number):
                                                       ris = {
                                                            ...
                                                            'MSG_SENDER_NAME':  '• ᴏᴜᴘʙᴏᴛs {}'.format(str(no)),
                                                            }
                                                       self.client.sendMessage(to, 'Responsename • ᴏᴜᴘʙᴏᴛs {}'.format(str(no)), contentMetadata=ris)
                                                       no += 1
                                                       time.sleep(0.05)
                                   else:
                                       sp = txt.split(" ")
                    ...
                    if txt == "squad list":
                        if self.status['squad']:
                            ...
                            if not silent:self.client.send_message(to,"List is empty.")
                    if txt == '@bye' or txt == 'retreat':
                        if self.bye == 1:
                            if not silent:self.client.send_message(to, "Bye {} ".format(self.client.get_group(to).name))
                    ...
                    if txt == 'backup list' and self.mode == 0:
                ...
                if self.ranking(to,saya) < 5:
                  ...
                  for a in cmds:
                    ...
                    if txt == 'set' or txt == "settings":
                        ...
                        #if self.settings["mode1"]: md+="• Mode1     → ✊\n"
                        #else: md+="• Mode2     → ✊\n"
                        if self.mode == 1:
                    ...
                    if txt.startswith('protect ') or txt == 'ready' or txt == 'close' or txt == 'protect':
                        if  txt.startswith('protect '):    
                            ...
                            if jenis == 'on' or jenis == '1':
                                ...
                                self.settings["protect"][to] = 1
                                if not silent:self.client.send_message(msg.to_id, "I'm Back.")
                        elif txt == 'close':
                                self.settings["protect"][to] = 0
                    ...
                    if self.status["invite"]:
                            if uid in self.status["blacklist"]:
                                ...
                                try:
                                    ...
                                    if not silent:self.client.send_message(to,"Limit cok")
                                    self.status["invite"] = False
